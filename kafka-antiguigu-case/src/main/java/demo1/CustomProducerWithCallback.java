...
public class CustomProducerWithCallback {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 100; i++) {
            producer.send(new ProducerRecord<>("helloworld", 0, Integer.toString(i), Integer.toString(i)), new Callback() {
                @Override
                public void onCompletion(RecordMetadata recordMetadata, Exception e) {
                    String str = e == null? "success":"error";
                    System.out.println(recordMetadata.topic()+"-"+recordMetadata.partition()+"-"+recordMetadata.offset()+" state: "+str);
                }
            });
        }
        producer.close();
